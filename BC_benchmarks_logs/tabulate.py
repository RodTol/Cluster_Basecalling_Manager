...
# CSV file path
csv_file_path_sup = "runs_sup.csv"
csv_file_path_hac = "runs_hac.csv"


# Write header to CSV file
with open(csv_file_path_sup, mode='w', newline='') as csv_file:
    fieldnames = ['setups', 'samples/s-sum', 'Gb/s-sum', 'total-time']
    writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
    writer.writeheader()

    # Iterate through the directory names
    for dir_name in dir_names_sup:
...
print(f"CSV file created: {csv_file_path_sup}")

#HAC--------------------------

# Write header to CSV file
with open(csv_file_path_hac, mode='w', newline='') as csv_file:
    fieldnames = ['setups', 'samples/s-sum', 'Gb/s-sum', 'total-time']
    writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
    writer.writeheader()

    # Iterate through the directory names
    for dir_name in dir_names_hac:
        # Construct the full path to the directory
        full_path = os.path.join("/u/dssc/tolloi/Cluster_Basecalling_Manager/BC_benchmarks_logs/", dir_name)
...
print(f"CSV file created: {csv_file_path_hac}")
