                ...
                if (len(str_dirname)>8 & (str_dirname[:8]=="ASSIGNED")) | (len(str_dirname)>6 & (str_dirname[:6]=="FAILED")):
                    for assignment in os.scandir(entry):
                        ...
                        if assignment.is_symlink():
                            ...
                            if len(str_assignment)>6 & (str_assignment[-6:].lower() == ".fast5"):
                                # add it to the list
                                assigned_files.append(str_assignment[:-6])
            elif entry.is_file():
                # It's a file: it should be a fast5 file that potentially needs to be processed unless already assigned
                str_name = entry.name
                if len(str_name)>6 & (str_name[-6:].lower() == ".fast5"):
                    str_name = str_name[:-6]
...
if __name__ == '__main__':
    ...
    RESTFulAPI.app.run(host='0.0.0.0', port=5000)
